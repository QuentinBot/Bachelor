...
def extract_text(directory):
    ...
    pattern_q = [{"TEXT": {"IN": pollutants_no_number}}, {"TEXT": ","}, {"TEXT": "corresponding"}, {"TEXT": "to"}, {"TEXT": "a"}, {"TEXT": {"REGEX": trend_number_regex}}]

    matcher.add("firstMatcher", [pattern, long_pattern, two_pattern], on_match=basic_pattern_match)
    matcher.add("no_poll_matcher", [pattern_c, pattern_l], on_match=no_pollutant_match)
    matcher.add("bracket_matcher", [pol_after_number_pattern, second_basic_pattern, pattern_a, pattern_b, long_pattern_2, pattern_d, pattern_e, pattern_f, pattern_m], on_match=bracket_matcher)
    matcher.add("multi_matcher", [multi_pattern, second_multi_pattern, two_pattern_reverse, pattern_i, pattern_j, pattern_o, pattern_g, pattern_p, pattern_r], on_match=multi_matcher)
    ...
    for file in directories:
        ...
        for pg in range(len(pdf)):
            ...
            # for tok in doc:
            #     if tok.text == "indicate" and tok.nbor().text == "widespread":
            #         for t in tok.sent:
